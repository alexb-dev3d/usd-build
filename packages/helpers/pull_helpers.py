...
def pull_zip(zip_url: str, package_name: str, download_dir: str, force_pull: bool = False, force_unzip: bool = False) -> bool:
    try:
        logger.info(f"Pulling repository:\n    - {zip_url}\n    - {download_dir}")
        out_dir = os.path.join(download_dir,package_name)

        splt = urlparse(zip_url)
        zip_name = splt.path.split('/')[-1]
        zip_name = os.path.join(download_dir, zip_name)
        pull = True
        ...
        logger.info(f"Checking if folder {out_dir} already exists")
...
def pull_package(package_config: PackageConfig, pull_dir: os.PathLike) -> bool:
    ...
    try:
        ...
        return pull_zip(url, package_name, pull_dir, force_pull = False, force_unzip = False)
    except Exception as e:
        logger.exception(f"Ooups: {e}")
        return False